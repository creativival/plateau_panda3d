...
class CelestialSphere:
    def __init__(self):
        sky_texture = self.settings['sky_texture']
        if sky_texture:
            self.sky_texture = self.loader.loadTexture(f'models/maps/sky/{sky_texture}')
            # 座標軸
            self.celestial_sphere_node = self.map_node.attachNewNode(PandaNode('axis_node'))
